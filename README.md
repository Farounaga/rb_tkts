...
### Erreur native `numo-narray` sur Ruby 3.4 (Windows)

Si `bundle install` échoue avec une erreur de compilation C sur `numo-narray`,
utilisez la configuration du dépôt (qui passe par `numo-narray-alt`) puis relancez :

```powershell
bundle config set force_ruby_platform true
bundle update numo-narray-alt rumale
bundle install
```

Si votre environnement reste instable, solution la plus simple et robuste :
- utiliser Ruby **3.3.x** (x64 mingw/ucrt),
- puis relancer `bundle install`.
